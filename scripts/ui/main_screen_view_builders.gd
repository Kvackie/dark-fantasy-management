...
static func build_debug_page(page_content: VBoxContainer, callbacks: Dictionary) -> void:
	...
	inventory_body.add_child(UIScreenHelpers.make_label("Adds 100 random item stacks to inventory.", 16))
	inventory_body.add_child(UIScreenHelpers.make_button("Generate 100 Random Items", callbacks.get("debug_grant_item", Callable()), false))
