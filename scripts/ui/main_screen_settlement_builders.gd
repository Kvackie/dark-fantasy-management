...
static func _build_empty_plot_panel(detail_content: VBoxContainer, slot_index: int, callbacks: Dictionary) -> void:
	...
	_add_text(detail_content, _txt("settlement.choose_structure"))
	for building_definition in GameManager.get_building_catalog():
		...
		body.add_child(UIScreenHelpers.make_label(String(building_data.get("name", "Unknown")), 18))
		body.add_child(UIScreenHelpers.make_label(String(building_data.get("description", "")), 13))
		...
		body.add_child(UIScreenHelpers.make_button("Build %s" % String(building_data.get("name", "Structure")), callbacks.get("build_selected_building", Callable()).bind(slot_index, String(building_data.get("id", ""))), not GameManager.can_afford(build_cost)))
...
static func _build_building_panel(detail_content: VBoxContainer, slot_index: int, slot_data: Dictionary, detail_mode: String, heroes_snapshot: Array, callbacks: Dictionary) -> void:
	...
	_add_text(detail_content, String(building_definition.get("description", "")))
	_add_text(detail_content, "Level %d  |  Workers %d / %d" % [
		int(slot_data.get("level", 1)),
		UIScreenHelpers.as_array(slot_data.get("assigned_hero_ids", [])).size(),
	...
	if not production_preview.is_empty():
		detail_content.add_child(_make_rich_text_label("[color=#d8d1c6]%s[/color] %s" % [_txt("settlement.production", {"production": ""}).trim_suffix(" "), _format_resource_bbcode(production_preview, "refund")], 15))
	...
	detail_content.add_child(_make_rich_text_label("[color=#d8d1c6]%s[/color] %s" % [_txt("settlement.upgrade_cost", {"cost": ""}).trim_suffix(" "), _format_resource_bbcode(upgrade_cost, "cost")], 15))
	_add_button(detail_content, _txt("settlement.upgrade_button"), callbacks.get("upgrade_slot", Callable()).bind(slot_index), at_max_level or not GameManager.can_afford(upgrade_cost))
	detail_content.add_child(_make_rich_text_label("[color=#d8d1c6]%s[/color] %s" % [_txt("settlement.dismantle_refund", {"refund": ""}).trim_suffix(" "), _format_resource_bbcode(dismantle_refund, "refund")], 15))
	_add_button(detail_content, _txt("settlement.dismantle_button"), callbacks.get("dismantle_slot", Callable()).bind(slot_index), false)
	...
	if not assigned_any:
		_add_text(detail_content, _txt("settlement.no_assigned_heroes"))
	...
	if available_heroes.is_empty():
		_add_text(detail_content, _txt("settlement.no_available_heroes"))
	else:
...
static func _add_hero_entry(detail_content: VBoxContainer, hero: Dictionary, assigned: bool, detail_mode: String, selected_slot: int, callbacks: Dictionary) -> void:
	...
	body.add_child(UIScreenHelpers.make_label(String(hero.get("name", "Unknown Hero")), 17))
	...
	if not assigned and assigned_slot >= 0 and not assigned_settlement_id.is_empty():
		...
		body.add_child(UIScreenHelpers.make_label(_txt("settlement.current_assignment", {"building": assignment_name}), 13))
	if detail_mode == "settlement" and selected_slot >= 0:
		if assigned:
			body.add_child(UIScreenHelpers.make_small_action_button(_txt("hero.unassign"), callbacks.get("unassign_hero", Callable()).bind(int(hero.get("uid", -1)))))
		else:
			var button_text := _txt("hero.move_here") if assigned_slot >= 0 and not assigned_settlement_id.is_empty() else _txt("hero.assign")
			body.add_child(UIScreenHelpers.make_small_action_button(button_text, callbacks.get("assign_hero", Callable()).bind(int(hero.get("uid", -1)), selected_slot)))
...
static func _add_section(detail_content: VBoxContainer, text: String) -> void:
	detail_content.add_child(UIScreenHelpers.make_label(text, 20))
