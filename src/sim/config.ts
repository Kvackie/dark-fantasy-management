/**
 * Game content, read from `src/data/` and normalised once.
 *
 * The JSON keeps the shape the original Godot build wrote, so it is tidied
 * here: missing fields get the defaults that loader gave them, ranges written
 * as `"8-14"` become `{ min, max }`, and entries that name no id are dropped.
 */
